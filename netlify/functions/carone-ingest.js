...
// Sin schedule: carone.com.mx tiene "User-agent: * / Disallow: /" en su
// robots.txt (verificado 2026-07-11), asi que este adaptador se deja listo
// pero inactivo. Reactivar el cron solo si Car One cambia su politica.

const MAX_RUNTIME_MS = 8 * 60 * 1000;

function runId() {
  return 'car1-' + new Date().toISOString()
    .replace(/[-:]/g, '')
    .replace('T', '-')
    .replace(/\.\d+Z$/, '')
    .slice(0, 17);
...
async function getCarOneSource() {
  const res = await sb(`agg_source_registry?name=eq.Car%20One&select=*&limit=1`);
  if (!res.ok || !Array.isArray(res.data) || !res.data[0]) {
    throw new Error(`carone_source_missing:${JSON.stringify(res.data).slice(0, 200)}`);
  }
  return res.data[0];
}
...
async function getLastProgress() {
  const res = await sb('agg_ingest_runs?source_name=eq.carone&select=detalle,started_at&order=started_at.desc&limit=1');
...
exports.handler = async (event = {}) => {
  ...
  const maxModelos = Number(qs.max_modelos || qs.maxModelos || 6);
  const pages = Math.min(Number(qs.pages || 4) || 4, 5);
  const perModelLimit = Math.min(Number(qs.per_model_limit || qs.perModelLimit || 6) || 6, 12);
  ...
  try {
    ...
    const [source, models, last] = await Promise.all([getCarOneSource(), getModels(200), getLastProgress()]);
    if (!models.length) throw new Error('agg_modelo_referencia no devolvio modelos');
    const startIndex = (last + 1) % models.length;
    const ordered = [...models.slice(startIndex), ...models.slice(0, startIndex)];
    ...
    for (let offset = 0; offset < ordered.length; offset++) {
      ...
      const absoluteIndex = (startIndex + offset) % models.length;
      ...
      let cars = [];
      try {
        cars = await withCrawlerFetch(() => carone.discover(marca, modelo, '', {
          limit: perModelLimit,
          pages,
          timeoutMs: 10000
        }));
