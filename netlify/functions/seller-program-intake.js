...
function normalizeListing(raw, lot) {
  ...
  const description = clean(raw.description || raw.descripcion || raw['descripción'] || 'Inventario autorizado para Tixuz Autos.', 1600);
  ...
  return {
    ...
    seller_name: clean(lot.name || raw.seller_name || raw.vendedor || 'Vendedor Tixuz Autos', 120),
...
async function createDraftListing({ listingData, endpoint, key }) {
  const res = await fetchWithTimeout(`${endpoint}/rest/v1/rpc/create_listing`, {
    ...
    body: JSON.stringify({
      ...
      p_seller_type: listingData.seller_type || 'Agencia',
      p_plan: 'basic',
...
exports.handler = async function(event) {
  ...
  if (created[0]) {
    notification = await notifyReviewCreated({ endpoint: SUPABASE_URL, key: SERVICE_KEY, listingId: created[0].id, event, source: 'seller-program-intake' }).catch(err => ({ attempted:false, reason:err.message }));
  }
